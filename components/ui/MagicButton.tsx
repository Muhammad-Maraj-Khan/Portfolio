...
const MagicButton = ({title, icon, position,handleClick, otherClasses}:{
    ...
    return (
        <button className="relative w-full inline-flex h-12 overflow-hidden rounded-lg p-[1px] focus:outline-none md:w-60 md:mt-10" onClick={handleClick}>
            ...
            <span className={`inline-flex h-full w-full gap-2 cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-7 text-sm font-medium text-white backdrop-blur-3xl`}>
                {position == 'left' && icon}
                {title}
                {position == 'right' && icon}
            </span>
